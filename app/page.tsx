...
import {
  ...
  obtenerEquiposNube,
  type EquipoGuardado,
} from "@/lib/equiposNube";
import {
  guardarPartidoNube,
  obtenerPartidosNube,
  type PartidoGuardado,
} from "@/lib/partidosNube";
import {
  BarChart,
  Bar,
  XAxis,
...
function getSectionColors(side: TeamCondition) {
  if (side === "local") {
    return {
      ...
      input:
        "border-2 border-blue-700 bg-white text-slate-950 placeholder:text-slate-500 focus:border-blue-900 focus:ring-2 focus:ring-blue-300",
  ...
  return {
    ...
    input:
      "border-2 border-red-700 bg-white text-slate-950 placeholder:text-slate-500 focus:border-red-900 focus:ring-2 focus:ring-red-300",
...
function getRefColors() {
  return {
    ...
    input:
      "border-2 border-yellow-500 bg-white text-slate-950 placeholder:text-slate-500 focus:border-yellow-700 focus:ring-2 focus:ring-yellow-300",
  };
}

function getResultBadgeClass(result: ResultState) {
  if (result === "G") return "bg-emerald-100 text-emerald-800 border border-emerald-300";
  if (result === "E") return "bg-slate-200 text-slate-700 border border-slate-300";
...
function getOpponentSuggestions(
  ...
  return unique
    .filter((name) => name.toLowerCase().includes(currentValue.trim().toLowerCase()))
    .slice(0, 12);
}

function getLevelLabel(value: number, high: number, medium: number): "Alto" | "Medio" | "Bajo" {
...
function getProfileBadgeClass(level: "Alto" | "Medio" | "Bajo") {
  if (level === "Alto") return "bg-emerald-100 text-emerald-800 border border-emerald-300";
  if (level === "Medio") return "bg-amber-100 text-amber-800 border border-amber-300";
...
function getCombinedReading(
  ...
  if (localProfile.estabilidad === "Volátil" || visitProfile.estabilidad === "Volátil") {
    riesgo = "Alto";
  } else if (
...
export default function AnalizadorApuestasPage() {
  ...
  const [partidosNube, setPartidosNube] = useState<PartidoGuardado[]>([]);
  const [cargandoPartidosNube, setCargandoPartidosNube] = useState(false);
  ...
  const partidos = useMemo(() => {
    const items: Array<{ equipo: string }> = [];
    if (matchInfo.local.trim()) items.push({ equipo: matchInfo.local.trim() });
    if (matchInfo.visitante.trim()) items.push({ equipo: matchInfo.visitante.trim() });
    return items;
  ...
  async function cargarPartidosNube() {
    try {
      setCargandoPartidosNube(true);
      const data = await obtenerPartidosNube();
      setPartidosNube((data ?? []) as PartidoGuardado[]);
    } catch (error) {
      console.error("Error cargando partidos:", error);
      alert("No se pudieron cargar los partidos guardados.");
    } finally {
      setCargandoPartidosNube(false);
    }
  }

  async function guardarPartidoCompletoEnNube() {
    try {
      const local = matchInfo.local?.trim();
      const visitante = matchInfo.visitante?.trim();

      if (!local || !visitante) {
        alert("Debes escribir el equipo local y visitante.");
        return;
      }
      ...
      await guardarPartidoNube({
        nombre_partido: nombrePartido,
        local,
        visitante,
        liga: matchInfo.liga ?? "",
        fecha_partido: matchInfo.fecha ?? "",
        arbitro: refInfo.nombre ?? "",
        payload,
      });

      await cargarPartidosNube();
      alert("Partido guardado completo en la nube.");
    } catch (error) {
      console.error("Error guardando partido completo:", error);
      alert("No se pudo guardar el partido completo.");
    }
  }

  function cargarPartidoGuardadoEnPantalla(partido: PartidoGuardado) {
    const data = partido.payload as {
      matchInfo?: MatchInfo;
      refInfo?: RefereeInfo;
      localRows?: TeamRow[];
      visitRows?: TeamRow[];
      odds?: typeof odds;
      parlay?: PickItem[];
      monteCarloResult?: typeof monteCarloResult;
    };

    if (data.matchInfo) setMatchInfo(data.matchInfo);
    if (data.refInfo) setRefInfo(data.refInfo);
    if (data.localRows) setLocalRows(data.localRows);
    if (data.visitRows) setVisitRows(data.visitRows);
    if (data.odds) setOdds(data.odds);
    if (data.parlay) setParlay(data.parlay);
    if (data.monteCarloResult) setMonteCarloResult(data.monteCarloResult);

    alert("Partido cargado en pantalla.");
  }
  ...
  const expectedGoalsLocal = useMemo(() => {
    ...
    const positionAdj =
      matchInfo.posicionLocal !== "" && matchInfo.posicionVisitante !== ""
        ? clamp(
            (Number(matchInfo.posicionVisitante) - Number(matchInfo.posicionLocal)) * 0.03,
            -0.35,
            0.35
          )
        : 0;
  ...
  const expectedGoalsVisit = useMemo(() => {
    ...
    const positionAdj =
      matchInfo.posicionLocal !== "" && matchInfo.posicionVisitante !== ""
        ? clamp(
            (Number(matchInfo.posicionLocal) - Number(matchInfo.posicionVisitante)) * 0.03,
            -0.35,
            0.35
          )
        : 0;
  ...
  const localProfile = useMemo(() => {
    ...
    if (localStats.totalGoalsStd >= 1.4 || localStats.cornersStd >= 3) estabilidad = "Variable";
    if (localStats.totalGoalsStd >= 1.8 || localStats.cornersStd >= 4) estabilidad = "Volátil";
    ...
    return { goles, btts, corners, tarjetas, estabilidad, estilo };
  }, [localStats]);
  ...
  const visitProfile = useMemo(() => {
    ...
    let estabilidad: "Estable" | "Variable" | "Volátil" = "Estable";
    if (visitStats.totalGoalsStd >= 1.4 || visitStats.cornersStd >= 3) estabilidad = "Variable";
    if (visitStats.totalGoalsStd >= 1.8 || visitStats.cornersStd >= 4) estabilidad = "Volátil";
  ...
  const trapAlert = useMemo(() => {
    ...
    if (volatilityLabel === "Alta" && Math.max(simulation.localWin, simulation.awayWin) < 55) {
      score += 2;
    ...
    if (score <= 2) {
      return { label: "Estable", color: "text-emerald-700 bg-emerald-50 border-emerald-200", reasons };
    }
    if (score <= 4) {
      return { label: "Cuidado", color: "text-amber-700 bg-amber-50 border-amber-200", reasons };
    }
  ...
  const bestPicks = useMemo<PickItem[]>(() => {
    ...
    const strongGoalSignal = expectedTotalGoals >= 2.4 && bttsProb >= 60;
    const mediumGoalSignal = expectedTotalGoals >= 2.2 && bttsProb >= 55;
    const lowGoalWarning = expectedTotalGoals < 2.2 && bttsProb < 55 && simulation.draw >= 20;
    ...
    const isBigMatch = Math.max(simulation.localWin, simulation.awayWin) >= 60 && volatilityLabel === "Baja";
      ...
      if (!(isBigMatch && !strongCornerSignal)) {
        picks.push({
          ...
          probabilidad: clamp(
            Math.max(
              corners85Prob,
              strongCornerSignal ? expectedTotalCorners * 8 : expectedTotalCorners * 10
            ),
            0,
            100
          ),
  ...
  }, [localStats, visitStats, refInfo.promedioAmarillas, simulation, expectedTotalGoals, expectedTotalCards, expectedTotalCorners, volatilityLabel]);
  ...
  const discardedMarkets = useMemo(() => {
    ...
    return items;
  }, [simulation, volatilityLabel, localStats, visitStats, expectedTotalGoals]);

  const profile = useMemo(() => {
    const openClosed =
      expectedTotalGoals >= 2.9 ? "Abierto" : expectedTotalGoals <= 2.1 ? "Cerrado" : "Intermedio";
    const cardsStyle =
      expectedTotalCards >= 5.5
        ? "Tarjetas altas"
        : expectedTotalCards <= 4.0
        ? "Tarjetas bajas"
        : "Tarjetas medias";
    const cornersStyle =
      expectedTotalCorners >= 9.3
        ? "Corners altos"
        : expectedTotalCorners <= 7.5
        ? "Corners bajos"
        : "Corners medios";
  ...
  function handleRowChange(side: TeamCondition, index: number, field: keyof TeamRow, value: string) {
  ...
  const avgParlayProb = parlay.length === 0 ? 0 : avg(parlay.map((p) => p.probabilidad));

  const valueComparison = [
    { mercado: "1", sistema: simulation.localWin, casa: impliedProb(odds.local) },
    { mercado: "X", sistema: simulation.draw, casa: impliedProb(odds.empate) },
    { mercado: "2", sistema: simulation.awayWin, casa: impliedProb(odds.visitante) },
    {
      mercado: "Más de 2.5 goles",
      ...
      <section className={`rounded-2xl border p-4 shadow-sm ${colors.wrapper}`}>
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              onClick={() =>
                side === "local" ? setLocalRows(createEmptyRows()) : setVisitRows(createEmptyRows())
              }
    ...
    <main className="min-h-screen bg-slate-100 p-4 md:p-6">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="xl:col-span-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <select
                ...
                className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 focus:border-slate-700 focus:ring-2 focus:ring-slate-300"
              >
                {STAGES.map((s) => (
              ...
              <div>
                <label className="mb-1 block text-sm font-semibold text-slate-700">Fecha del partido</label>
                <input
                  type="date"
                  value={matchInfo.fecha}
                  onChange={(e) => setMatchInfo((prev) => ({ ...prev, fecha: e.target.value }))}
                  className="w-full rounded-xl border-2 border-slate-400 bg-white px-3 py-2 font-semibold text-slate-900 focus:border-slate-600 focus:ring-2 focus:ring-slate-300"
                />
              </div>
              <input
                ...
                className="rounded-xl border px-3 py-2 text-slate-900 md:col-span-2"
              />
              <textarea
                placeholder="Notas del partido"
                value={matchInfo.notas}
                onChange={(e) => setMatchInfo((p) => ({ ...p, notas: e.target.value }))}
                className="min-h-[90px] rounded-xl border px-3 py-2 text-slate-900 md:col-span-2"
              />
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm">
          <h2 className="text-xl font-bold text-emerald-700">Partidos guardados en nube</h2>
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              type="button"
              onClick={guardarPartidoCompletoEnNube}
          ...
          <div className="mt-4 space-y-2">
            {cargandoPartidosNube ? (
              <p className="text-sm text-slate-500">Cargando partidos...</p>
            ) : partidosNube.length === 0 ? (
              <p className="text-sm text-slate-500">No hay partidos guardados todavía.</p>
            ) : (
              partidosNube.map((partido) => (
                <div key={partido.id} className="rounded-xl border border-slate-200 p-3">
                  <p className="font-semibold text-slate-800">{partido.nombre_partido}</p>
                  <p className="text-sm text-slate-500">
                    {partido.liga || "Sin liga"} · {partido.fecha_partido || "Sin fecha"}
                  </p>
                  <button
                    onClick={() => cargarPartidoGuardadoEnPantalla(partido)}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="text-xl font-bold text-slate-800">Panel visual del partido</h2>
          <p className="mt-1 text-sm text-slate-500">
            Vista rápida de las probabilidades principales del 1X2.
          </p>
          <div className="mt-4 h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartProbData}>
        ...
        <section className="grid gap-6 xl:grid-cols-3">
          <div className="xl:col-span-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-4">
              <input type="number" placeholder="GANA LOCAL" value={odds.local} onChange={(e) => setOdds((p) => ({ ...p, local: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
              <input type="number" placeholder="EMPATE" value={odds.empate} onChange={(e) => setOdds((p) => ({ ...p, empate: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
              <input type="number" placeholder="GANA VISITANTE" value={odds.visitante} onChange={(e) => setOdds((p) => ({ ...p, visitante: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
              <input type="number" placeholder="Cuota Over +2.5" value={odds.over25} onChange={(e) => setOdds((p) => ({ ...p, over25: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
              <input type="number" placeholder="Cuota BTTS (AMBOS MARCAN)" value={odds.btts} onChange={(e) => setOdds((p) => ({ ...p, btts: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
              <input type="number" placeholder="Cuota corners +8.5" value={odds.corners85} onChange={(e) => setOdds((p) => ({ ...p, corners85: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
              <input type="number" placeholder="Cuota tarjetas +4.5" value={odds.cards45} onChange={(e) => setOdds((p) => ({ ...p, cards45: e.target.value === "" ? "" : Number(e.target.value) }))} className="rounded-xl border-2 border-slate-500 bg-white px-3 py-2 font-semibold text-slate-950 placeholder:text-slate-500 focus:border-slate-700 focus:ring-2 focus:ring-slate-300" />
            </div>
              ...
              <table className="min-w-[720px] w-full text-sm">
                <thead>
                  <tr className="bg-slate-100 text-left text-slate-700">
                    <th className="border px-3 py-2 text-slate-900">Mercado</th>
                    <th className="border px-3 py-2 text-slate-900">Sistema</th>
                    <th className="border px-3 py-2 text-slate-900">Casa</th>
                    <th className="border px-3 py-2 text-slate-900">Diferencia</th>
                    <th className="border px-3 py-2 text-slate-900">Valor</th>
                  </tr>
                </thead>
                <tbody>
                  {valueComparison.map((row) => {
                    ...
                    return (
                      <tr key={row.mercado}>
                        <td className="border px-3 py-2 text-slate-900">{row.mercado}</td>
                        <td className="border px-3 py-2 text-slate-900">{formatPct(row.sistema)}</td>
                        <td className="border px-3 py-2 text-slate-900">{formatPct(row.casa)}</td>
                        <td className="border px-3 py-2 text-slate-900">{diff.toFixed(1)}%</td>
                        <td className="border px-3 py-2 font-semibold text-slate-900">{diff >= 5 ? "✔ Sí" : "❌ No"}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border-2 border-blue-400 bg-blue-50 p-4">
              <h3 className="text-lg font-bold text-blue-900">{matchInfo.local || "Equipo local"}</h3>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  <p className="mb-1 text-sm font-semibold text-blue-900">Goles</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(localProfile.goles)}`}>{localProfile.goles}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-blue-900">BTTS</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(localProfile.btts)}`}>{localProfile.btts}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-blue-900">Corners</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(localProfile.corners)}`}>{localProfile.corners}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-blue-900">Tarjetas</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(localProfile.tarjetas)}`}>{localProfile.tarjetas}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-blue-900">Estabilidad</p>
                  <span className="inline-flex rounded-lg border border-blue-300 bg-white px-2 py-1 text-sm font-bold text-blue-900">{localProfile.estabilidad}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-blue-900">Estilo</p>
                  <span className="inline-flex rounded-lg border border-blue-300 bg-white px-2 py-1 text-sm font-bold text-blue-900">{localProfile.estilo}</span>
                </div>
              </div>
            </div>

            <div className="rounded-2xl border-2 border-red-400 bg-red-50 p-4">
              <h3 className="text-lg font-bold text-red-900">{matchInfo.visitante || "Equipo visitante"}</h3>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div>
                  <p className="mb-1 text-sm font-semibold text-red-900">Goles</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(visitProfile.goles)}`}>{visitProfile.goles}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-red-900">BTTS</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(visitProfile.btts)}`}>{visitProfile.btts}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-red-900">Corners</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(visitProfile.corners)}`}>{visitProfile.corners}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-red-900">Tarjetas</p>
                  <span className={`inline-flex rounded-lg px-2 py-1 text-sm font-bold ${getProfileBadgeClass(visitProfile.tarjetas)}`}>{visitProfile.tarjetas}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-red-900">Estabilidad</p>
                  <span className="inline-flex rounded-lg border border-red-300 bg-white px-2 py-1 text-sm font-bold text-red-900">{visitProfile.estabilidad}</span>
                </div>
                <div>
                  <p className="mb-1 text-sm font-semibold text-red-900">Estilo</p>
                  <span className="inline-flex rounded-lg border border-red-300 bg-white px-2 py-1 text-sm font-bold text-red-900">{visitProfile.estilo}</span>
                </div>
              </div>
            </div>
          </div>
